    ...
    <div class="main-container">
        <div class="categories-list-main">
            @foreach ($categories as $category)
                <a href="{{ route('showCategory', $category->id) }}">{{ $category->name }}
                    ({{ $category->countItems() }})
                </a>
            @endforeach
        </div>
        <div class="items-list-main">
            @foreach ($items as $item)
                <div class="item-main">
                    ...
                    @endif
                    <h4 class="price-main">{{ $item->price }} рубчинских</h4>
                    @if (Auth::check())
                        <form id="add_to_cart-form" action="{{ route('storeCart') }}" method="post">
                            @csrf
                            <input type="hidden" name="item_id" value="{{ $item->id }}">
                            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                            <button class="add-to-cart" type="submit">Добавить в корзину<svg width="24" height="24"
